...
const validator = (req,res,next) =>{
    const schema = joi.object({
        name: joi.string().required().trim().min(3).max(30).pattern(new RegExp('[A-Za-z]')).messages({
        ...
        lastName: joi.string().required().trim().min(3).max(35).pattern(new RegExp('[A-Za-z]')).messages({
